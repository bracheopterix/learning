...
const textArray = lowerText.matchAll(/\w+/g);
...
for (let [key, value] of Object.entries(wordsCounter)){      // SAMI LEV PO
    if (varCount < value){
        varCount = value;
        varName = key;
    }
...
console.log('The word', varName, 'apperaed', varCount, 'times');



/* 
Дан текст. Посчитать частоту всех слов. 
Подсказка: использовать ключи объекта, слова определяются регулярным выражением 
/\w+/g   - синтаксис регулярных выражений. / - начало и конец выражения, \w = буква или цифра, 
а + = “один и более”. g = модификатор, говорящий что регулярное выражение должно применяться глобально. 
То есть найти не первое вхождение, а все, которые есть в тексте.
Текст взять несколько страниц, например рассказ бредбери.

var -> lower caseing -> array (regEx) -> obj -> iterations
iter = every new word in var if there wasn`t any, if was - previous var+1`.array

each word -> if exists +1, else - creating var (keys in wC?) -> output
*/